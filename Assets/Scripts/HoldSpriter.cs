...
public class HoldSpriter : MonoBehaviour
{
    ...
    public void WriteSprite_debug()
    {
        ...
        for (int p = 0; p < points.Length; p++)
        {
            ...
            {
                Debug.DrawLine(pos[tras[i]], pos[tras[i + 1]], Color.yellow, 0.001f);
                Debug.DrawLine(pos[tras[i + 1]], pos[tras[i + 2]], Color.yellow, 0.001f);
                Debug.DrawLine(pos[tras[i + 2]], pos[tras[i]], Color.yellow, 0.001f);
            }
